...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id 44B3A5EB72C
	for <lists+linux-fscrypt@lfdr.de>; Tue, 27 Sep 2022 03:49:39 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S230032AbiI0Bti (ORCPT <rfc822;lists+linux-fscrypt@lfdr.de>);
        Mon, 26 Sep 2022 21:49:38 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:37646 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S230004AbiI0Bth (ORCPT
        <rfc822;linux-fscrypt@vger.kernel.org>);
        Mon, 26 Sep 2022 21:49:37 -0400
Received: from dfw.source.kernel.org (dfw.source.kernel.org [IPv6:2604:1380:4641:c500::1])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 6BF8A2BD1;
        Mon, 26 Sep 2022 18:49:31 -0700 (PDT)
Received: from smtp.kernel.org (relay.kernel.org [52.25.139.140])
        (using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
        (No client certificate requested)
        by dfw.source.kernel.org (Postfix) with ESMTPS id E667661529;
        Tue, 27 Sep 2022 01:49:30 +0000 (UTC)
Received: by smtp.kernel.org (Postfix) with ESMTPSA id 92854C433B5;
        Tue, 27 Sep 2022 01:49:30 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
        s=k20201202; t=1664243370;
        bh=PwUOTXAyjebiI+B4R5EFI5DfhyvSP4eCMWcE/X1E4iU=;
        h=From:To:Cc:Subject:Date:In-Reply-To:References:From;
        b=
...
Subject: [RFC PATCH v6 4/4] fscrypt: add support for hardware-wrapped keys
Date:   Mon, 26 Sep 2022 18:47:18 -0700
Message-Id: <20220927014718.125308-5-ebiggers@kernel.org>
...
index 5ba5817c17c2..e68741d78ef8 100644
...
@@ -498,6 +528,8 @@ This structure must be initialized as follows:
     policies`_.
...
@@ -697,7 +729,8 @@ a pointer to struct fscrypt_add_key_arg, defined as follows::
...
@@ -716,7 +749,7 @@ a pointer to struct fscrypt_add_key_arg, defined as follows::
...
@@ -785,6 +824,8 @@ FS_IOC_ADD_ENCRYPTION_KEY can fail with the following errors:
...
@@ -796,7 +837,9 @@ FS_IOC_ADD_ENCRYPTION_KEY can fail with the following errors:
...
+  hardware-wrapped keys
 ...
 Legacy method
 ~~~~~~~~~~~~~
@@ -859,9 +902,8 @@ or removed by non-root users.
...
+discussion of the security goals and limitations of these ioctls.
 ...
 ~~~~~~~~~~~~~~~~~~~~~~~~~~~~
@@ -1181,7 +1223,8 @@ inline encryption hardware doesn't have the needed crypto capabilities
...
@@ -1189,7 +1232,84 @@ inline encryption hardware that supports that data unit size.
...
index d5f68a0c5d15..180fda1c2291 100644
...
@@ -342,11 +366,17 @@ fscrypt_using_inline_encryption(const struct fscrypt_info *ci)
...
@@ -383,7 +413,8 @@ fscrypt_using_inline_encryption(const struct fscrypt_info *ci)
...
+				 bool is_hw_wrapped,
 ...
 {
 	WARN_ON(1);
@@ -396,6 +427,15 @@ fscrypt_destroy_inline_crypt_key(struct super_block *sb,
 {
 }
...
+
 ...
 fscrypt_is_key_prepared(struct fscrypt_prepared_key *prep_key,
 			const struct fscrypt_info *ci)
@@ -412,11 +452,23 @@ fscrypt_is_key_prepared(struct fscrypt_prepared_key *prep_key,
...
@@ -424,8 +476,14 @@ struct fscrypt_master_key_secret {
 	 */
 	u32			size;
...
index 5e8dabbc083e..decca8edd294 100644
--- a/fs/crypto/inline_crypt.c
+++ b/fs/crypto/inline_crypt.c
@@ -96,6 +96,7 @@ int fscrypt_select_encryption_impl(struct fscrypt_info *ci)
 {
...
@@ -121,8 +122,7 @@ int fscrypt_select_encryption_impl(struct fscrypt_info *ci)
...
@@ -133,7 +133,9 @@ int fscrypt_select_encryption_impl(struct fscrypt_info *ci)
...
+		BLK_CRYPTO_KEY_TYPE_HW_WRAPPED : BLK_CRYPTO_KEY_TYPE_STANDARD;
 	...
 	if (IS_ERR(devs))
@@ -156,11 +158,15 @@ int fscrypt_select_encryption_impl(struct fscrypt_info *ci)
...
+		BLK_CRYPTO_KEY_TYPE_HW_WRAPPED : BLK_CRYPTO_KEY_TYPE_STANDARD;
 	...
 	struct block_device **devs;
 	unsigned int num_devs;
@@ -172,7 +178,7 @@ int fscrypt_prepare_inline_crypt_key(struct fscrypt_prepared_key *prep_key,
 		return -ENOMEM;
...
@@ -232,6 +238,56 @@ void fscrypt_destroy_inline_crypt_key(struct super_block *sb,
...
index 1cca09aa43f8..e5a7b511531c 100644
...
@@ -532,20 +532,45 @@ static int add_master_key(struct super_block *sb,
 	int err;
...
+		err = fscrypt_hkdf_expand(&secret->hkdf, keyid_kdf_ctx, NULL, 0,
 		...
 		if (err)
@@ -554,19 +579,36 @@ static int add_master_key(struct super_block *sb,
...
@@ -613,18 +655,18 @@ static struct key_type key_type_fscrypt_provisioning = {
...
+static int get_keyring_key(u32 key_id, u32 type, u32 flags,
 ...
 {
 	key_ref_t ref;
@@ -641,8 +683,12 @@ static int get_keyring_key(u32 key_id, u32 type,
 		goto bad_key;
...
@@ -708,15 +754,24 @@ int fscrypt_ioctl_add_key(struct file *filp, void __user *_uarg)
 		return -EACCES;
...
+		if (!fscrypt_valid_key_size(arg.raw_size, arg.flags))
 		...
 		err = -EFAULT;
@@ -744,13 +799,13 @@ EXPORT_SYMBOL_GPL(fscrypt_ioctl_add_key);
 static void
...
@@ -764,7 +819,8 @@ int fscrypt_get_test_dummy_key_identifier(
...
index f7407071a952..695ddde96391 100644
--- a/fs/crypto/keysetup.c
+++ b/fs/crypto/keysetup.c
@@ -138,7 +138,9 @@ int fscrypt_prepare_key(struct fscrypt_prepared_key *prep_key,
...
+							false, ci);
 	...
 	if (IS_ERR(tfm))
@@ -179,14 +181,29 @@ static int setup_per_mode_enc_key(struct fscrypt_info *ci,
...
@@ -198,6 +215,16 @@ static int setup_per_mode_enc_key(struct fscrypt_info *ci,
 	if (fscrypt_is_key_prepared(prep_key, ci))
 		goto done_unlock;
...
@@ -320,6 +347,19 @@ static int fscrypt_setup_v2_file_key(struct fscrypt_info *ci,
 {
 	int err;
...
@@ -346,7 +386,7 @@ static int fscrypt_setup_v2_file_key(struct fscrypt_info *ci,
 		   FSCRYPT_POLICY_FLAG_IV_INO_LBLK_32) {
...
@@ -463,6 +503,14 @@ static int setup_file_encryption_key(struct fscrypt_info *ci,
...
index 46757c3052ef..7d496bd5f56c 100644
--- a/fs/crypto/policy.c
+++ b/fs/crypto/policy.c
@@ -207,7 +207,8 @@ static bool fscrypt_supported_v2_policy(const struct fscrypt_policy_v2 *policy,
...
+			      FSCRYPT_POLICY_FLAG_HW_WRAPPED_KEY)) {
 		fscrypt_warn(inode, "Unsupported encryption flags (0x%02x)",
 			     policy->flags);
 		return false;
@@ -222,6 +223,14 @@ static bool fscrypt_supported_v2_policy(const struct fscrypt_policy_v2 *policy,
 		return false;
 	}
...
index a756b29afcc2..68f435e5acbb 100644
