...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id 2E6745EB72E
	for <lists+linux-fscrypt@lfdr.de>; Tue, 27 Sep 2022 03:49:40 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S230008AbiI0Bti (ORCPT <rfc822;lists+linux-fscrypt@lfdr.de>);
        Mon, 26 Sep 2022 21:49:38 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:37644 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S230003AbiI0Bth (ORCPT
        <rfc822;linux-fscrypt@vger.kernel.org>);
        Mon, 26 Sep 2022 21:49:37 -0400
Received: from dfw.source.kernel.org (dfw.source.kernel.org [IPv6:2604:1380:4641:c500::1])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 5FC522738;
        Mon, 26 Sep 2022 18:49:31 -0700 (PDT)
Received: from smtp.kernel.org (relay.kernel.org [52.25.139.140])
        (using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
        (No client certificate requested)
        by dfw.source.kernel.org (Postfix) with ESMTPS id 7550D6150E;
        Tue, 27 Sep 2022 01:49:30 +0000 (UTC)
Received: by smtp.kernel.org (Postfix) with ESMTPSA id A94D1C433D6;
        Tue, 27 Sep 2022 01:49:29 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
        s=k20201202; t=1664243369;
        bh=yWFaZJP1BKxVlutVeMXYhEXwrPLnjE83PijY315pUKs=;
        h=From:To:Cc:Subject:Date:In-Reply-To:References:From;
        b=
...
Subject: [RFC PATCH v6 1/4] blk-crypto: add basic hardware-wrapped key support
Date:   Mon, 26 Sep 2022 18:47:15 -0700
Message-Id: <20220927014718.125308-2-ebiggers@kernel.org>
...
---
 ...
 11 files changed, 412 insertions(+), 24 deletions(-)

diff --git a/Documentation/block/inline-encryption.rst b/Documentation/block/inline-encryption.rst
index 4d151fbe2058..c1968a06344b 100644
...
index 621abd1b0e4d..fc268a435f47 100644
...
index e6818ffaddbf..5958ad9cc7f6 100644
...
index 96c511967386..a14fbdff28b6 100644
...
@@ -463,6 +465,60 @@ bool blk_crypto_register(struct blk_crypto_profile *profile,
 }
...
+
  ...
  *					 by child device
@@ -486,10 +542,12 @@ void blk_crypto_intersect_capabilities(struct blk_crypto_profile *parent,
...
@@ -522,6 +580,9 @@ bool blk_crypto_has_capabilities(const struct blk_crypto_profile *target,
...
@@ -556,5 +617,6 @@ void blk_crypto_update_capabilities(struct blk_crypto_profile *dst,
...
index a496aaef85ba..1980d132e419 100644
--- a/block/blk-crypto.c
+++ b/block/blk-crypto.c
@@ -22,18 +22,21 @@ const struct blk_crypto_mode blk_crypto_modes[] = {
 		.name = "AES-256-XTS",
 		.cipher_str = "xts(aes)",
 		.keysize = 64,
...
@@ -69,9 +72,15 @@ static int __init bio_crypt_ctx_init(void)
...
@@ -310,8 +319,9 @@ int __blk_crypto_rq_bio_prep(struct request *rq, struct bio *bio,
 /**
...
+ * @key_type: type of the key -- either standard or hardware-wrapped
  ...
  *	       key is used
@@ -320,7 +330,9 @@ int __blk_crypto_rq_bio_prep(struct request *rq, struct bio *bio,
...
@@ -333,8 +345,19 @@ int blk_crypto_init_key(struct blk_crypto_key *blk_key, const u8 *raw_key,
 		return -EINVAL;
...
+	}
 	...
 	if (dun_bytes == 0 || dun_bytes > mode->ivsize)
 		return -EINVAL;
@@ -345,9 +368,10 @@ int blk_crypto_init_key(struct blk_crypto_key *blk_key, const u8 *raw_key,
...
@@ -360,8 +384,10 @@ int blk_crypto_init_key(struct blk_crypto_key *blk_key, const u8 *raw_key,
...
+	return __blk_crypto_cfg_supported(q->crypto_profile, cfg);
 ...
 /**
@@ -384,6 +410,10 @@ int blk_crypto_start_using_key(const struct blk_crypto_key *key,
 {
...
index 332f96b58252..56d63b14d7c6 100644
...
index c40bd55bc781..5e8dabbc083e 100644
--- a/fs/crypto/inline_crypt.c
+++ b/fs/crypto/inline_crypt.c
@@ -133,6 +133,7 @@ int fscrypt_select_encryption_impl(struct fscrypt_info *ci)
...
+	crypto_cfg.key_type = BLK_CRYPTO_KEY_TYPE_STANDARD;
 	...
 	if (IS_ERR(devs))
@@ -170,7 +171,8 @@ int fscrypt_prepare_inline_crypt_key(struct fscrypt_prepared_key *prep_key,
 	if (!blk_key)
 		return -ENOMEM;
...
index bbab65bd5428..8dc769eaf2e0 100644
...
index 69b24fe92cbf..c30907c19303 100644
...
+			enum blk_crypto_key_type key_type,
 			...
 			unsigned int dun_bytes,
 			unsigned int data_unit_size);
@@ -103,6 +159,13 @@ int blk_crypto_evict_key(struct request_queue *q,
 bool blk_crypto_config_supported(struct request_queue *q,
 				 const struct blk_crypto_config *cfg);
