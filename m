...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id B4CC35995D7
	for <lists+linux-fscrypt@lfdr.de>; Fri, 19 Aug 2022 09:21:11 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S242189AbiHSHSR (ORCPT <rfc822;lists+linux-fscrypt@lfdr.de>);
        Fri, 19 Aug 2022 03:18:17 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:49250 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S241528AbiHSHSQ (ORCPT
        <rfc822;linux-fscrypt@vger.kernel.org>);
        Fri, 19 Aug 2022 03:18:16 -0400
Received: from dfw.source.kernel.org (dfw.source.kernel.org [139.178.84.217])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 3913E60D8;
        Fri, 19 Aug 2022 00:18:12 -0700 (PDT)
Received: from smtp.kernel.org (relay.kernel.org [52.25.139.140])
        (using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
        (No client certificate requested)
        by dfw.source.kernel.org (Postfix) with ESMTPS id 787836151C;
        Fri, 19 Aug 2022 07:18:11 +0000 (UTC)
Received: by smtp.kernel.org (Postfix) with ESMTPSA id 9B681C433D7;
        Fri, 19 Aug 2022 07:18:10 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
        s=k20201202; t=1660893490;
        bh=I1voyK+emlj0xjGPL0OHugZmSus71r77VaJgUlaGsRM=;
        h=From:To:Cc:Subject:Date:In-Reply-To:References:From;
        b=
...
Subject: [PATCH 1/2] fscrypt: stop using keyrings subsystem for fscrypt_master_key
Date:   Fri, 19 Aug 2022 00:15:31 -0700
Message-Id: <20220819071532.221026-2-ebiggers@kernel.org>
...
---
 ...
 fs/crypto/keyring.c         | 467 +++++++++++++++++++-----------------
 fs/crypto/keysetup.c        |  81 +++----
 fs/crypto/policy.c          |   8 +-
 fs/super.c                  |   2 +-
 include/linux/fs.h          |   2 +-
 include/linux/fscrypt.h     |   4 +-
 8 files changed, 330 insertions(+), 307 deletions(-)
...
index caee9f8620dd9b..74f841ccae7589 100644
...
@@ -38,20 +51,69 @@ static void move_master_key_secret(struct fscrypt_master_key_secret *dst,
...
+	 * been wiped when the last active reference was dropped; doing it here
+	 * would be unnecessarily late.  Nevertheless, use kfree_sensitive() in
...
+	if (!refcount_dec_and_lock(&mk->mk_active_refs, &keyring->lock))
+		return;
+	/*
+	 * No active references left, so complete the removal of this
...
@@ -61,44 +123,6 @@ static inline bool valid_key_spec(const struct fscrypt_key_specifier *spec)
...
-
 static int fscrypt_user_key_instantiate(struct key *key,
 					struct key_preparsed_payload *prep)
 {
@@ -131,32 +155,6 @@ static struct key_type key_type_fscrypt_user = {
...
@@ -164,21 +162,6 @@ static struct key *search_fscrypt_keyring(struct key *keyring,
...
@@ -199,20 +182,15 @@ static void format_mk_user_description(
...
@@ -222,21 +200,75 @@ static int allocate_filesystem_keyring(struct super_block *sb)
 	return 0;
 }
...
+{
+	/*
+	 * As the key specifiers should already be random values, it is
+	 * sufficient to use a trivial hash function that just takes the first
+	 * several bits of the key specifier.
+	 */
...
@@ -246,10 +278,19 @@ struct key *fscrypt_find_master_key(struct super_block *sb,
 	 */
...
+		if (memcmp(&mk->mk_spec, mk_spec, sizeof(*mk_spec)) == 0 &&
...
@@ -277,17 +318,30 @@ static int allocate_master_key_users_keyring(struct fscrypt_master_key *mk)
...
+ * write, or the master key must be still undergoing initialization.
 ...
 {
@@ -309,7 +363,7 @@ static int add_master_key_user(struct fscrypt_master_key *mk)
...
+ * ->mk_sem must be held for write.
  ...
  */
@@ -327,63 +381,49 @@ static int remove_master_key_user(struct fscrypt_master_key *mk)
 }
...
@@ -392,42 +432,34 @@ static int add_new_master_key(struct fscrypt_master_key_secret *secret,
...
@@ -436,38 +468,37 @@ static int do_add_master_key(struct super_block *sb,
 			     const struct fscrypt_key_specifier *mk_spec)
 {
...
-			goto retry;
+			/*
+			 * Oops, we found a key struct with no secret, but the
+			 * key struct was removed from the keyring by the time
+			 * we tried to re-add the secret.  We have to ignore the
+			 * old struct and add a new one.  fscrypt_add_key_mutex
+			 * means we don't need to worry about concurrent adds.
+			 */
+			err = add_new_master_key(sb, secret, mk_spec);
 		}
-		key_put(key);
...
@@ -771,19 +802,19 @@ int fscrypt_verify_key_added(struct super_block *sb,
...
+	down_read(&mk->mk_sem);
 	...
 	if (IS_ERR(mk_user)) {
 		err = PTR_ERR(mk_user);
@@ -791,7 +822,8 @@ int fscrypt_verify_key_added(struct super_block *sb,
 		key_put(mk_user);
 		err = 0;
 	}
-	key_put(key);
...
+	fscrypt_put_master_key(mk);
 out:
 	if (err == -ENOKEY && capable(CAP_FOWNER))
 		err = 0;
@@ -953,11 +985,10 @@ static int do_remove_key(struct file *filp, void __user *_uarg, bool all_users)
...
+	bool inodes_remain;
 	...
 	if (copy_from_user(&arg, uarg, sizeof(arg)))
 		return -EFAULT;
@@ -977,12 +1008,10 @@ static int do_remove_key(struct file *filp, void __user *_uarg, bool all_users)
 		return -EACCES;
...
@@ -991,7 +1020,7 @@ static int do_remove_key(struct file *filp, void __user *_uarg, bool all_users)
 		else
...
@@ -1003,26 +1032,22 @@ static int do_remove_key(struct file *filp, void __user *_uarg, bool all_users)
 			status_flags |=
...
+	if (inodes_remain) {
 		...
 		if (err == -EBUSY) {
@@ -1038,7 +1063,7 @@ static int do_remove_key(struct file *filp, void __user *_uarg, bool all_users)
...
+	fscrypt_put_master_key(mk);
 	...
 	return err;
@@ -1085,7 +1110,6 @@ int fscrypt_ioctl_get_key_status(struct file *filp, void __user *uarg)
 {
 	struct super_block *sb = file_inode(filp)->i_sb;
...
@@ -1102,19 +1126,18 @@ int fscrypt_ioctl_get_key_status(struct file *filp, void __user *uarg)
 	arg.user_count = 0;
...
+			FSCRYPT_KEY_STATUS_ABSENT /* raced with remove */;
 		err = 0;
 		goto out_release_key;
 	}
@@ -1136,8 +1159,8 @@ int fscrypt_ioctl_get_key_status(struct file *filp, void __user *uarg)
 	}
 	err = 0;
 out_release_key:
-	up_read(&key->sem);
...
+	fscrypt_put_master_key(mk);
 out:
 	if (!err && copy_to_user(uarg, &arg, sizeof(arg)))
 		err = -EFAULT;
@@ -1149,13 +1172,9 @@ int __init fscrypt_init_keyring(void)
 {
 	int err;
...
+		return err;
 	...
 	if (err)
@@ -1165,7 +1184,5 @@ int __init fscrypt_init_keyring(void)
...
index fbc71abdabe324..94dda72ef9cf3f 100644
...
+		 * inode from a master key that already had its secret removed,
+		 * then complete the full removal of the master key.
 		 */
